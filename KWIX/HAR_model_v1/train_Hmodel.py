...
def hparam_model(hparams, x_data, y_data, test_xdata, test_ydata):
    ...
    model.fit(
        ...
        callbacks=[earlystopping,
                   ModelCheckpoint('C://Users/kwuser/Desktop/vscode/JH/kwix/HAR/models/model.h5',
    ...
    for num_units in HP_NUM_UNITS.domain.values:
        for dropout_rate in (HP_DROPOUT.domain.min_value, HP_DROPOUT.domain.max_value):
            for earlystop_rate in (HP_EARLYSTOP.domain.min_value, HP_EARLYSTOP.domain.max_value):
                path_dir1 = 'JH/kwix/HAR/dataset'
                folder_list1 = os.listdir(path_dir1)
                path_dir2 = 'JH/kwix/HAR/dataset_test'
                ...
                data = np.concatenate([
                    np.load(path_dir1 + '/' + folder_list1[4]),
                    np.load(path_dir1 + '/' + folder_list1[5]),
                    np.load(path_dir1 + '/' + folder_list1[6]),
                    np.load(path_dir1 + '/' + folder_list1[7])
                ], axis=0)
                
                test_data = np.concatenate([
                    np.load(path_dir2 + '/' + folder_list2[4]),
                    np.load(path_dir2 + '/' + folder_list2[5]),
                    np.load(path_dir2 + '/' + folder_list2[6]),
                    np.load(path_dir2 + '/' + folder_list2[7])
                ], axis=0)
